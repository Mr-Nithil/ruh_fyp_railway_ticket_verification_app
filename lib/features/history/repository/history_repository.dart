...
class HistoryRepository {
  ...
  Future<List<BookingDetails>?> fetchCheckingHistoryByCheckerId(
    String checkerId,
    String scheduleId,
  ) async {
    try {
      ...
      final query = '''
        ...
        WHERE bdi."CheckedBy" = @checkerId 
          AND b."ScheduleId" = @scheduleId 
          AND b."Deleted" = false
        ORDER BY bdi."CheckedOn" DESC NULLS LAST, bp."IsDependent" ASC NULLS LAST;
      ''';

      // Execute query with parameters
      final result = await dbService.connection.execute(
        Sql.named(query),
        parameters: {'checkerId': checkerId, 'scheduleId': scheduleId},
      );
      ...
      print('❌ Error fetching booking by checker ID: $e');
      await dbService.close();
      rethrow;
    }
  }
